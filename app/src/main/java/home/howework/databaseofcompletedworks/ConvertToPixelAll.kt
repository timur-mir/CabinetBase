...
fun convertToPixels(context: Context, nDP: Int): Float {
    ...
    if(density==1.6f){
        resultValue=(nDP * density*4 + 0.5f).toFloat()
    }
    if(density==2.0f){
        resultValue=(nDP * density*3 + 0.5f).toFloat()
    }
    else if(density==3.0f)
    {
        resultValue=(nDP * density*2.4 + 0.5f).toFloat()
    }
    else if(density==2.5f)
    {
        resultValue=(nDP * density*3.3 + 0.5f).toFloat()
    }
...
fun convertToPixels28(context: Context, nDP: Int): Float {
    val conversionScale:Int = context.getResources().getDisplayMetrics().densityDpi
